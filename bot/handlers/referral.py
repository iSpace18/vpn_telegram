...
async def referral_info(message: Message, db_session: AsyncSession, bot: Bot):
    ...
    text = (
        ...
        f"Бонусы можно использовать для оплаты подписки (в разработке)."
    )
...
async def inline_referral(inline_query: InlineQuery, db_session: AsyncSession, bot: Bot):
    ...
    if not user:
        result = InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(message_text="Нажмите кнопку, чтобы запустить бота и получить реферальную ссылку."),
